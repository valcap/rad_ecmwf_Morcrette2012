...
! LDYNCORE=.T.    switches on idealized planet
! LAQUA=.T.       switches on idealized aqua planet
! LHELDSUAREZ=.T. switches on idealized Held-Suarez
! LVERTINI=.T.    allows to provide different set of A,Bs in namelist
!                 (!only works with idealized initial data!)
...
! small planet factor
REAL(KIND=JPRB)     :: RPLRADI
! Coriolis acceleration factor
REAL(KIND=JPRB)     :: RCORIOI

! The following options are implemented 
! N3DINI == 2 == analysed initial data from file, (orog == 0., moisture depends
!                on LAQUA=T/F)
! N3DINI == 3 == idealized GATE initial profiles (after Grabowski (2001) JAS,
!                58, 978--997)
!                (moisture depends on LAQUA=T/F)
! N3DINI == 4 == no/zonal flow, neutral stratification (theta == constant)
! N3DINI == 5 == no/zonal flow, stratified isothermal atmosphere (T == constant) 

! NTESTCASE = 0 Williamson mountain, case 1;
!               Williamson et al. (1992) JCP, 102, 211--224
! NTESTCASE = 1 Williamson mountain, case 5;
!               Williamson et al. (1992) JCP, 102, 211--224
! NTESTCASE = 2 Klemp-Schaer mountain profile;
!               Klemp, Skamarock, Fuhrer (2003) MWR, 131, 1229-1239
! NTESTCASE = 3 simple mountain, low froude number flow
! NTESTCASE = 4 elliptic (quasi-2D) Witch of Agnesi setup for tropos-stratos NH,
!               T. Keller (1994) JAS, 51, 1915--1929
! NTESTCASE = 5 marginally resolved step mountain (like Himalaya)
! NTESTCASE = 6 profile and wind shear following
!               T. Keller (1994) JAS, 51, 1915--1929
! NTESTCASE = 7 critical flow LS2 following Grubisic and Smolarkiewicz (1997)
!               JAS, 1943--1960
! NTESTCASE = 8 critical flow LS5 following Grubisic and Smolarkiewicz (1997)
!               JAS, 1943--1960
! NTESTCASE = 9 acoustic wave (following Landau/Lifschitz)
! NTESTCASE = 10 sea breeze, to be done.
! NTESTCASE = 11 double bubble test following Robert (1993) JAS, 1865--1873
! NTESTCASE = 12 single rising bubble test following Robert (1993)
!                JAS, 1865--1873
...
! DRAGX      -  mountain drag in zonal direction
! DRAGY      -  mountain drag in meridional direction
! DRAGNORM   -  asymptotic linear mountain drag
!REAL(KIND=JPRB), ALLOCATABLE :: DRAGX(:)
!REAL(KIND=JPRB), ALLOCATABLE :: DRAGY(:)
REAL(KIND=JPRB) :: DRAGNORM
