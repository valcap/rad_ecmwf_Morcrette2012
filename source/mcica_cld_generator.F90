SUBROUTINE MCICA_CLD_GENERATOR &
  &( PZF    , PAVG_CF, PAVG_QLW  , PAVG_QIW, PGLAT, PGLON, &
...
USE YOE_McICA,ONLY : NMcI1, NMcI2, XCW

!-- Mike's random number generator
USE YOMCT3  , ONLY : NSTEP
USE YOMDYN  , ONLY : TSTEP
USE YOMRIP  , ONLY : NINDAT
...
REAL(KIND=JPRB), INTENT(IN) :: &
  & PZF(KLON,KLEV),        & ! Full-level (layer midpoint) altitude  (km)
...
!Print 9001,NINDAT,NSTEP,TSTEP,IDAY,ITIM
!9001 format(1x,'Generator_1 ',2I10,F8.1,2I10)
...
DO IL = KIDIA, KFDIA
  DO JK=I_TOP(IL),I_BASE(IL)-1
    IF (PRLC_CF(IL,JK) > 0.0) THEN
      ZTMP1 = (PZF(IL,JK) - PZF(IL,JK+1)) / PRLC_CF(IL,JK)
!      IF (ZTMP1 < 0._JPRB .OR. ZTMP1 > 200._JPRB) THEN
!        ISTOP=ISTOP+1
!      ENDIF
!      ZALPHA(IL,JK) = EXP(-(PZF(IL,JK) - PZF(IL,JK+1)) / PRLC_CF(IL,JK))
      ZALPHA(IL,JK) = EXP(-ZTMP1)
    ELSE
      ZALPHA(IL,JK) = 0.0
    ENDIF
    IF (PRLC_CW(IL,JK) > 0.0) THEN
      ZTMP2 = (PZF(IL,JK) - PZF(IL,JK+1)) / PRLC_CW(IL,JK)
!      IF (ZTMP2 < 0._JPRB .OR. ZTMP2 > 200._JPRB) THEN
!        ISTOP=ISTOP+1
!      ENDIF
!      ZRCORR(IL,JK) = EXP(-(PZF(IL,JK) - PZF(IL,JK+1)) / PRLC_CW(IL,JK))
      ZRCORR(IL,JK) = EXP(-ZTMP2)
    ELSE
      ZRCORR(IL,JK) = 0.0
    ENDIF
!    IF (ISTOP > 0) THEN
!      PRINT 9002,ISTOP,IL,JK,ZTMP1,ZTMP2,PZF(IL,JK),PZF(IL,JK+1),PRLC_CF(IL,JK),PRLC_CW(IL,JK),ZALPHA(IL,JK),ZRCORR(IL,JK)
...
ENDDO
!Print 9003,ISEED,KIDIA,KFDIA,(PGLON(IL),PGLAT(IL),IL=KIDIA,KFDIA)
!9003 format(1x,'Generator_3 ',I16,2I5,(24F6.1))
!!Print 9004,(YL_RANDOM_STREAM(IL),IL=KIDIA,KFDIA)
!!9004 format(1x,'Generator_4 ',10I16)

DO I=1,KNX_LOC

  DO IL = KIDIA, KFDIA
!    ISEED=NINT( PGLON(IL)+PGLAT(IL)+ITIM+IDAY+ I )
