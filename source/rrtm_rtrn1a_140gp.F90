...
REAL(KIND=JPRB)   ,INTENT(IN)    :: P_ABSS1(KIDIA:KFDIA,JPGPT*JPLAY) !!!! Andrea
REAL(KIND=JPRB)   ,INTENT(IN)    :: P_OD(JPGPT,JPLAY,KIDIA:KFDIA)
REAL(KIND=JPRB)   ,INTENT(IN)    :: P_TAUSF1(KIDIA:KFDIA,JPGPT*JPLAY) !!!! Andrea
  ...
  DO JLEV = KLEV, 1, -1
    ...
    IF (K_ICLDLYR(JLON,JLEV) == 1) THEN
      ...
      DO JI = 1, JPGPT
        ...
        Z_BBU1(INDEX,JLON) = Z_BGLAY + P_TAUSF1(JLON,INDEX) * Z_DELBGUP
        !--DS
        Z_BGLEV(JI) = PFRAC(JLON,JI,JLEV) * Z_PLVL(JLON,NGB(JI),JLEV-1)
        !----
        Z_DELBGDN = Z_BGLEV(JI) - Z_BGLAY
        Z_BBD = Z_BGLAY + P_TAUSF1(JLON,INDEX) * Z_DELBGDN
        ...
        Z_ATOT1(INDEX,JLON) = P_ABSS1(JLON,INDEX) + Z_ABSCLDNW(JI,JLEV)&
         & - P_ABSS1(JLON,INDEX) * Z_ABSCLDNW(JI,JLEV)
        Z_BBDTOT = Z_BGLAY + Z_FACTOT1 * Z_DELBGDN
        Z_GASSRC = Z_BBD * P_ABSS1(JLON,INDEX)
        !***
        ...
        Z_CLRRADD(JI) = Z_CLRRADD(JI) * (1.0_JPRB-P_ABSS1(JLON,INDEX)) +&
        ...
        Z_RADCLRD1(JI,JLON) = Z_RADCLRD1(JI,JLON)+(Z_BBD-Z_RADCLRD1(JI,JLON))*P_ABSS1(JLON,INDEX)
        ...
        !   cloudy streams
        Z_RADMOD = Z_RAD(JI) * (Z_FACCLR1D(JLON,JLEV-1) * (1.0_JPRB-P_ABSS1(JLON,INDEX)) +&
    ...
    ELSE
      ...
      IF (ICLDDN == 1) THEN
        DO JI = 1, JPGPT
          ...
          !- total-sky downward radiance
          Z_RADLD1(JI,JLON) = Z_RADLD1(JI,JLON)+(Z_BBD-Z_RADLD1(JI,JLON))*P_ABSS1(JLON,INDEX)
          Z_DRAD1__ = Z_DRAD1__ + Z_RADLD1(JI,JLON)
  ...
  DO JLEV = 1, KLEV
    ...
    DO JI = 1, JPGPT
      ...
      !- *** Cloudy layer
        ...
        Z_GASSRC = Z_BBU1(INDEX,JLON) * P_ABSS1(JLON,INDEX)
        ...
        !    cloudy streams depending on cloud fraction
          ...
          Z_RAD(JI) = 0.0_JPRB
        Z_TTOT = 1.0_JPRB - Z_ATOT1(INDEX,JLON)
        Z_TRNS = 1.0_JPRB - P_ABSS1(JLON,INDEX)
    ...
    DO JI = 1, JPGPT
      INDEX = IENT + JI
        ...
        !- total-sky upward flux
        Z_RADLU1(JI,JLON) = &
       &Z_RADLU1(JI,JLON)+(Z_BBU1(INDEX,JLON)-Z_RADLU1(JI,JLON))*P_ABSS1(JLON,INDEX)
        Z_URAD1_(JLON,JLEV) = &
       &Z_URAD1_(JLON,JLEV) + Z_RADLU1(JI,JLON)
        ...
        Z_RADCLRU1(JI,JLON) = &
       &Z_RADCLRU1(JI,JLON)+(Z_BBU1(INDEX,JLON)-Z_RADCLRU1(JI,JLON))*P_ABSS1(JLON,INDEX)
