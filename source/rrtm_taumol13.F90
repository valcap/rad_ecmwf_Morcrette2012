...
USE PARRRTM  , ONLY : JPLAY  ,JPBAND ,JPGPT  ,NG13  ,NGS12
USE YOERRTWN , ONLY : NSPA   
USE YOERRTA13, ONLY : ABSA   ,FRACREFA,SELFREF,STRRAT
...
DO JLAY = 1, KLEV
  DO JLON = KIDIA, KFDIA
    IF (JLAY <= K_LAYTROP(JLON)) THEN
      Z_SPECCOMB(JLAY) = P_COLH2O(JLON,JLAY) + STRRAT*P_COLN2O(JLON,JLAY)
      ...
      IND1(JLAY) = (K_JP(JLON,JLAY)*5+(K_JT1(JLON,JLAY)-1))*NSPA(13) + JS
...
!    FAC111 = FS * FAC11(LAY)
!----
        P_TAU(JLON,NGS12+JG,JLAY) = Z_SPECCOMB(JLAY) *&
     !-- DS_000515
     ...
     !-- DS_000515
        ...
        PFRAC(JLON,NGS12+JG,JLAY) = FRACREFA(JG,JS) + Z_FS *&
...
!CDIR UNROLL=NG13
      DO JG = 1, NG13
!-- JJM_000517
        P_TAU(JLON,NGS12+JG,JLAY) = P_TAUAERL(JLON,JLAY,13)
        PFRAC(JLON,NGS12+JG,JLAY) = 0.0_JPRB
      ENDDO
    ENDIF
  ENDDO
ENDDO
