...
DO JLAY = 1, KLEV
  DO JLON = KIDIA, KFDIA
    IF (JLAY <= K_LAYTROP(JLON)) THEN
      Z_SPECCOMB(JLAY) = P_COLH2O(JLON,JLAY) + STRRAT*P_COLO3(JLON,JLAY)
      ...
      IND1(JLAY) = (K_JP(JLON,JLAY)*5+(K_JT1(JLON,JLAY)-1))*NSPA(7) + JS
...
!-----
        ...
        P_TAU(JLON,NGS6+JG,JLAY) = Z_SPECCOMB(JLAY) *&
     !-- DS_000515
     ...
     !-- DS_000515
        ...
        PFRAC(JLON,NGS6+JG,JLAY) = FRACREFA(JG,JS) + Z_FS *&
...
!CDIR UNROLL=NG7
      DO JG = 1, NG7
!-- JJM_000517
        P_TAU(JLON,NGS6+JG,JLAY) = P_COLO3(JLON,JLAY) *&
         ...
         & + P_CO2MULT(JLON,JLAY) * ABSCO2(JG)&
         & + P_TAUAERL(JLON,JLAY,7)  
        PFRAC(JLON,NGS6+JG,JLAY) = FRACREFB(JG)
      ENDDO
    ENDIF
  ENDDO
ENDDO
